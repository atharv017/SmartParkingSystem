...
parkingRouter.post("/", async (req, res) => {
    try {
        let { name, address, city, user_id } = req.body
        ...
        const { error } = schema.validate({ name, address, city, user_id });
        if (error) {
            res.status(400).json({ error: error.details[0].message });
        }
        else {
            const parking = await Parking.create({ name, address, city, user_id });
    ...
    try {
        ...
        if (Types.ObjectId.isValid(id)) {
            ...
            if (!parking) {
                ...
                let { name, address, city, user_id } = parking;
                user_id = user_id.toString()
                const updatedParkingObj = { name, address, city, user_id, ...req.body }
